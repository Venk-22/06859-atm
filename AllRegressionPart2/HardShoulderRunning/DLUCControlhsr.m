function[]= DLUCControlhsr(runNumber, mainlineDemand, bottleneckSpeed)
...
control_steps=(time_duration)/dt; %Be careful to add +1 here, edit
...
detector1=detector_set{1}; % detector on first lane
...
for i=1:control_steps
    ...
    if occ_Avg>Thres_Occ %if average occupancy greater than threshold then open lane
		HSRsig=1;
        display(['SHR - Open at ' num2str(real_time) ' seconds; Average occupancy: ' num2str(occ_Avg)])
        %for k=1:length(link_set)
        temp_link=link_set{3};
            %if length(temp_link.Lanes.GetAll)==4
        temp_link.Lanes.GetAll{4}.set('AttValue','BlockedVehClasses','60'); % block bike
            %end
        %end
        
    else
        HSRsig=0;
        display(['SHR - Closed at ' num2str(real_time) ' seconds; Average occupancy: ' num2str(occ_Avg)])
        %for k=1:length(link_set)
        temp_link1=link_set{3}; %select the link of interest
        temp_link1.Lanes.GetAll{4}.set('AttValue','BlockedVehClasses',vehclass_Str); %block the vehicles of interest on rightmost lane
            %end
        %end
    end
    ...
    for j=1:(dt*sim_resolution)
        ...
        avgOcc= (occupancy_estimate1+occupancy_estimate2+occupancy_estimate3)/3;
        occ_Rec=[occ_Rec avgOcc];
    end
